...
// Ícones sincronizados exatamente com o menu lateral do aplicativo
const QUICK_NAV = [
  { to: "/", label: "Início", Icon: Home, color: "bg-slate-100 text-slate-700" },
  { to: "/ia-cidade", label: "IA da Cidade", Icon: Bot, color: "bg-emerald-50 text-emerald-600" },
  { to: "/anuncios", label: "Anúncios", Icon: Megaphone, color: "bg-amber-50 text-amber-600" },
  { to: "/empresas", label: "Empresas", Icon: Building2, color: "bg-blue-50 text-blue-600" },
  { to: "/servicos", label: "Serviços", Icon: Wrench, color: "bg-violet-50 text-violet-600" },
  { to: "/noticias", label: "Notícias", Icon: Newspaper, color: "bg-sky-50 text-sky-600" },
  { to: "/eventos", label: "Eventos", Icon: CalendarDays, color: "bg-pink-50 text-pink-600" },
  { to: "/saude", label: "Saúde", Icon: HeartPulse, color: "bg-red-50 text-red-600" },
  { to: "/turismo", label: "Turismo", Icon: Camera, color: "bg-amber-50 text-amber-700" },
  { to: "/prefeitura", label: "Prefeitura", Icon: Landmark, color: "bg-slate-100 text-slate-800" },
  { to: "/fale-conosco", label: "Fale Conosco", Icon: Mail, color: "bg-teal-50 text-teal-700" },
];
...
function Index() {
  ...
  return (
    <AppLayout>
      {/* Greeting + Banner da Cidade com Maior Visibilidade */}
      <section className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-center">
        <div>
        ...
        {/* Banner ampliado e com forte presença visual */}
        <div className="relative w-full h-[180px] rounded-2xl overflow-hidden ring-1 ring-slate-200 shadow-md group">
          <div 
          ...
          <div className="absolute bottom-4 left-4 text-white">
            <span className="bg-emerald-600 text-white text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider mb-1 inline-block">
              Portal Oficial
            </span>
            <p className="text-base font-bold">São Brás do Suaçuí</p>
            <p className="text-xs opacity-90">Nossa cidade, nosso orgulho</p>
          </div>
        </div>
      </section>
      ...
      {/* Categories / Acesso Rápido Sincronizado com o Menu */}
      <section>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-11 gap-2.5">
          {QUICK_NAV.map(({ to, label, Icon, color }) => (
            <Link
              key={to}
              to={to}
              className="group bg-white rounded-xl ring-1 ring-slate-200 p-3 flex flex-col items-center text-center gap-2 hover:shadow-md hover:-translate-y-0.5 transition-all"
            >
              <div className={`size-10 rounded-lg flex items-center justify-center ${color}`}>
                <Icon className="size-5" />
              </div>
              <span className="text-[11px] font-semibold text-slate-700 leading-tight line-clamp-1">{label}</span>
            </Link>
          ))}
        </div>
      </section>
